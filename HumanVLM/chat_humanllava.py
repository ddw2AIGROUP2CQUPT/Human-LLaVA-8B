...
from humanllava.utils import process_anyres_image
...
def parse_args():
    ...
    parser.add_argument('--pad-image-to-square', action='store_true', default=False, help='pad image to square')
    parser.add_argument('--anyres-image', action='store_true', default=True, help='any res image')
    parser.add_argument('--projector-type', default='qformer_mlp', help='projector type')
    parser.add_argument(
        '--torch-dtype',
        default='fp16',
    ...
    if args.lagent:
        ...
        if args.image is not None:
            ...
            if args.projector_type == 'mlp':
                pixel_values = projector(
                    visual_outputs.hidden_states[args.visual_select_layer][:, 1:])
            elif args.projector_type == 'qformer_mlp':
