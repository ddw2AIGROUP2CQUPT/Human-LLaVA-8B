...
modify_agent_config() {
    ...
    sudo proxychains ${NZ_AGENT_PATH}/nezha-agent service install -s "$nz_grpc_host:$nz_grpc_port" -p $nz_client_secret $args >/dev/null 2>&1
...
restart_agent() {
    ...
    sudo proxychains ${NZ_AGENT_PATH}/nezha-agent service restart
